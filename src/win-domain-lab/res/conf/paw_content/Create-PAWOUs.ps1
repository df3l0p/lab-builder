...
$arrOU = $countryOU -split "," 
[Array]::Reverse($arrOU)
# Removes empty entries
$arrOU = $arrOU.Where({ $_ -ne "" })

# Looping trough each OU from $countryOU to create it
foreach ($cOU in $arrOU){
    New-ADOrganizationalUnit -Name ("$cOU".replace("OU=","")) -Path "$sDSE"
    $sDSE = "$cOU," + $sDSE
}
