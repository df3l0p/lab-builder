...
# On a production environement, uncomment all #$forestDnsZonesACL lines
# because it is not working on vagrant environment for an unknown reason
...
$serviceDeskOperatorsGroup = "${prefixOUSamAccountName}ServiceDeskOperators"
$workstationMaintenanceGroup = "${prefixOUSamAccountName}WorkstationMaintenance"
$replicationMaintenanceGroup = "${prefixOUSamAccountName}Tier0ReplicationMaintenance"
$tier1ServerMaintenanceGroup = "${prefixOUSamAccountName}Tier1ServerMaintenance"
$PAWAdminsGroup = "${prefixOUSamAccountName}PAWMaint"

#Get a reference to each of the OU's we want to set permissions on
#Add-Log -LogEntry("Getting OU Information");
$userAcctsOUDN = Get-ADOrganizationalUnit -Identity ($userAccountsOU+$countryOU+$domain)
$workstationsOUDN = Get-ADOrganizationalUnit -Identity ($workstationsOU+$countryOU+$domain)
$computerQuarantineOUDN = Get-ADOrganizationalUnit -Identity ($computerQuarantineOU+$countryOU+$domain)
$tier1ServersOUDN = Get-ADOrganizationalUnit -Identity ($tier1ServersOU+$countryOU+$domain)
$PAWDevicesOUDN = Get-ADOrganizationalUnit -Identity ($PAWDevicesOU+$countryOU+$domain)
...
Set-ACL -ACLObject $configContainerACL -Path ("AD:\"+($configCN)) -ErrorAction SilentlyContinue;
Set-ACL -ACLObject $schemaNCACL -Path ("AD:\"+($schemaNC)) -ErrorAction SilentlyContinue;
#Set-ACL -ACLObject $forestDnsZonesACL -Path ("AD:\"+($forestDnsZonesDN));
Set-ACL -ACLObject $sitesACL -Path ("AD:\"+($sitesDN)) -ErrorAction SilentlyContinue;
